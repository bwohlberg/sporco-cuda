...
__global__ void
cuda_solvedbi_sm_vec4(float2 * Out, float2 * ah, float2 * Dsf,
   float rho, float2 * YUf, float2 * c, int nRows, int nCols, int nFilts)
{
  ...
  if ((Tidx < nCols) & (Tidy < half_nRows)) {
    ...
    for (k = 0; k < nFilts; k += 1) {
      ...
      cb = cuCaddf_vec4 (cb, cuCmulf_vec4(b,
...
__global__ void
cuda_solvedbi_sm (float2 * Out, float2 * ah, float2 * Dsf, float rho,
   float2 * YUf, float2 * c, int nRows, int nCols, int nFilts)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void
cuda_Shrink_CalU_vec4(float *Y, float *U, float *X, float lambda, int nRows,
   int nCols, int nFilts)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void
cuda_Shrink_vec4(float *Y, float *X, float *U, float lambda, int nRows,
   int nCols, int nFilts)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void
cuda_CalU_vec4(float *U, float *X, float *Y, int nRows, int nCols,
   int nFilts)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void
cuda_Cal_Dsf_C_vec4(float2 * Dsf, float2 * C, float2 * Sf, float2 * Df,
...
__global__ void
cuda_Cal_Dsf_C (float2 * Dsf, float2 * C, float2 * Sf, float2 * Df, float rho,
...
__global__ void
cuda_Cal_C_vec4(float2 * C, float2 * Df, float rho, int nRows, int nCols,
   int nFilts)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void
cuda_Cal_C (float2 * C, float2 * Df, float rho, int nRows, int nCols,
   int nFilts)
{
...
__global__ void
cuda_Pad_Dict (float *PadD, float *D, int nCols_D, int nRows_D, int nFilts,
   int nRows, int nCols)
{
...
__global__ void
cuda_CalYU_vec4(float *YU, float *Y, float *U, int nRows, int nCols,
   int nFilts)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void
cuda_OverRelax_vec4(float *Xr, float *X, float *Y, float RelaxParam,
   int nRows, int nCols, int nFilts)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void
cuda_Cal_residuals_norms_vec4(float *s, float *r, float *nX, float *nY,
   float *nU, float *X, float *Y, float *U, float *Yprv, float rho, int nRows,
   int nCols, int nFilts)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y;
  ...
  if (Tidx < nCols) {
    ...
    for (int i = Tidy; i < Part_Dim; i += stride) {
      ...
      sumX +=
	 ((X_temp.x * X_temp.x + X_temp.y * X_temp.y) + (X_temp.z * X_temp.z +
	    X_temp.w * X_temp.w));
      sumY +=
	 ((Y_temp.x * Y_temp.x + Y_temp.y * Y_temp.y) + (Y_temp.z * Y_temp.z +
	    Y_temp.w * Y_temp.w));
      sumU +=
	 ((U_temp.x * U_temp.x + U_temp.y * U_temp.y) + (U_temp.z * U_temp.z +
	    U_temp.w * U_temp.w));
	 ...
	 ((X_temp.x * X_temp.x + X_temp.y * X_temp.y) + (X_temp.z * X_temp.z +
	    X_temp.w * X_temp.w));
      sumYY +=
	 (((rho * Y_temp.x * Y_temp.x) + (rho * Y_temp.y * Y_temp.y)) +
...
__global__ void
cuda_norm_vec4(float *nX, float *nY, float *nU, float *X, float *Y, float *U,
   int nRows, int nCols, int nFilts)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void
cuda_Cal_residuals_vec4(float *s, float *r, float *X, float *Y, float *Yprv,
   float rho, int nRows, int nCols, int nFilts)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void
cuda_Fidelity_Term_vec4(float *Jdf, float2 * Df, float2 * Xf, float2 * Sf,
   int nRows, int nCols, int nFilts)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void
cuda_Fidelity_Term (float *Jdf, float2 * Df, float2 * Xf, float2 * Sf,
   int nRows, int nCols, int nFilts)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void
cuda_L1_Term_vec4(float *d_JL1, float *X, float L1Weight, int factor,
   int nRows, int nCols, int nFilts)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void
cuda_L1_Term_vec4_Vector(float *d_JL1, float *X, float* L1Weight, int factor,
   int nRows, int nCols, int nFilts)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y, index;
...
__global__ void
cuda_L1_Term_Vector(float *d_JL1, float *X, float* L1Weight, int factor,
   int nRows, int nCols, int nFilts)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y, index;
...
__global__ void
cuda_Cal_Gfw (float *GfW, float2 * Grf, float2 * Gcf, int nRows, int nCols)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y;
  unsigned int index;
  ...
  if ((Tidx < nCols) & (Tidy < nRows)) {
    ...
    Gcf_temp = Gcf[index];
    GfW_temp =
       Grf_temp.x * Grf_temp.x + Grf_temp.y * Grf_temp.y +
...
__global__ void
cuda_solvedbd_sm_vec4(float2 * Out, float2 * ah, float2 * Dsf, float *GfW,
   float *GrdWeight, float rho, float mu, float2 * YUf, float2 * c, int nRows,
...
__global__ void
cuda_solvedbd_sm (float2 * Out, float2 * ah, float2 * Dsf, float *GfW,
   float *GrdWeight, float rho, float mu, float2 * YUf, float2 * c, int nRows,
   int nCols, int nFilts, int nWeights)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
...
__global__ void
cuda_Fidelity_Gr_Term_vec4(float *Jdf, float *Jgr, float2 * Df, float2 * Xf,
   float2 * Sf, float *GfW, float *GrdWeight, int nRows, int nCols,
   int nFilts, int nWeights)
{
  ...
  if ((Tidx < nCols) & (Tidy < half_nRows)) {
	...
	Weight = GrdWeight[k];
      ...
      aux.x =
	 GfW_temp.x * Weight * (Xf_temp.x * Xf_temp.x +
	 Xf_temp.y * Xf_temp.y);
      aux.y =
	 GfW_temp.y * Weight * (Xf_temp.z * Xf_temp.z +
	 Xf_temp.w * Xf_temp.w);
...
__global__ void
cuda_Fidelity_Gr_Term (float *Jdf, float *Jgr, float2 * Df, float2 * Xf,
   float2 * Sf, float *GfW, float *GrdWeight, int nRows, int nCols,
   int nFilts, int nWeights)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  ...
  if ((Tidx < nCols) & (Tidy < nRows)) {
	...
	Weight = GrdWeight[k];
      ...
      aux =
	 GfW_temp * Weight * (Xf_temp.x * Xf_temp.x + Xf_temp.y * Xf_temp.y);
...
__global__ void
cuda_Cal_Dsf_grd_C_vec4(float2 * Dsf, float2 * C, float2 * Sf, float2 * Df,
   float *GfW, float *GrdWeight, float rho, float mu, int nRows, int nCols,
   int nFilts, int nWeights)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void
cuda_Cal_Dsf_grd_C (float2 * Dsf, float2 * C, float2 * Sf, float2 * Df,
   float *GfW, float *GrdWeight, float rho, float mu, int nRows, int nCols,
   int nFilts, int nWeights)
{
...
__global__ void
cuda_Cal_grd_C_vec4(float2 * C, float2 * Df, float *GfW, float *GrdWeight,
   float rho, float mu, int nRows, int nCols, int nFilts, int nWeights)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y;
  unsigned int index, k;
...
__global__ void
cuda_Cal_grd_C (float2 * C, float2 * Df, float *GfW, float *GrdWeight,
   float rho, float mu, int nRows, int nCols, int nFilts, int nWeights)
{
  unsigned int Tidx = threadIdx.x + blockIdx.x * blockDim.x;
  unsigned int Tidy = threadIdx.y + blockIdx.y * blockDim.y;
