...
void default_opts(void *data){
 ...
 AlgOpt *opt = ((AlgOpt *) data);
  ...
  if (opt->device == -1)         opt->device = 0;

  if (opt->rho == -1)            opt->rho = 0;
  ...
  if (opt->NonNegCoef == -1)     opt->NonNegCoef = 0;
  if (opt->RelaxParam == -1)     opt->RelaxParam = 1.8;
  if (opt->RhoScaling == -1)     opt->RhoScaling = 100;

  if (opt->RelStopTol  == -1)    opt->RelStopTol = 1e-4;
  if (opt->MaxMainIter == -1)    opt->MaxMainIter = 250;

  if (opt->NoBndryCross == -1)   opt->NoBndryCross = 0;
  if (opt->HighMemSolve == -1)   opt->HighMemSolve = 0;
  if (opt->StdResiduals == -1)   opt->StdResiduals = 0;
  if (opt->RhoRsdlRatio == -1)   opt->RhoRsdlRatio = 1.2;

  if (opt->RhoRsdlTarget == -1)  opt->RhoRsdlTarget = -1;
  if (opt->AutoRhoPeriod == -1)  opt->AutoRhoPeriod = 1;
  if (opt->AutoRhoScaling == -1) opt->AutoRhoScaling = 1;
}
...
cudaError_t get_current_device(int* dev) {
  return cudaGetDevice(dev);
}


cudaError_t set_current_device(int dev) {
