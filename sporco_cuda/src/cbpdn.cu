...
//extern "C"
void
cuda_wrapper_CBPDN (float *D, float *S, float lambda, void *vopt, float *Y)
{
  ...
  cuda_Pad_Dict <<< blocksPerGrid_Padding, threadsPerBlock >>>
		(d_aux, d_D, DICT_ROW_SIZE, DICT_COL_SIZE,
		 DICT_M_SIZE, IMG_ROW_SIZE, IMG_COL_SIZE);
  ...
  while (i <= MAX_ITER && (r > epri || s > edua)) {
    ...
    if (IMG_ROW_SIZE % 2) {
      cuda_solvedbi_sm <<< blocksPerGrid_fft, threadsPerBlock >>>
	    (d_auxf, d_Df, d_Dsf, rho, d_auxf, d_C, IMG_ROW_SIZE,
	     LIMIT_X, DICT_M_SIZE);
    } else {
	...
	checkCudaErrors(cudaMallocManaged(&d_Jdf, ((int) sizeof (float))));}
	  ...
	  if (IMG_ROW_SIZE % 2)
	    ...
	    cuda_Fidelity_Term_vec4 <<< blocksPerGrid_radix2_fft,
	      threadsPerBlock >>> (d_Jdf, d_Df, d_auxf, d_Sf,
				   IMG_ROW_SIZE, LIMIT_X, DICT_M_SIZE);
